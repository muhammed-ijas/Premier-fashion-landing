            ...
            {production.markets.map((market, i) => (
                <div key={market.name}>
                    <div className="flex items-baseline justify-between">
                        ...
                        <span className="text-[0.85rem] font-bold text-green">{market.share}%</span>
                    </div>
                ...
                <div className="mt-10 grid grid-cols-1 items-stretch gap-6 lg:grid-cols-[1.4fr_1fr] lg:gap-8">
                    ...
                    <div className="flex flex-col gap-6">
                        ...
                        <Stagger className="border border-line bg-white px-5 py-6 md:px-6">
                            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 sm:gap-12">
                                ...
                                <Stagger.Item as="up">
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        {portfolio.map((item) => (
                                            <div key={item.label}>
                                                <p className="text-2xl font-bold leading-none text-blue">{item.share}</p>
                            ...
                            <p className="mt-5 border-t border-line pt-3 text-[0.7rem] leading-[1.6] text-fg-subtle">
                                {portfolioNote}
                            </p>
                        </Stagger>
                            ...
                            <div className={SPLIT}>
                                {leadTimes.map((lead) => (
                                    <Stagger.Item key={lead.days} as="up">
                                        <p className="whitespace-nowrap text-2xl font-bold leading-none text-blue">
                                            {lead.days}
                                            <span className="ml-1.5 text-[0.6rem] font-semibold uppercase tracking-[0.08em] text-fg-subtle">
                                                days
                                            </span>
                                        </p>
                                        <p className="mt-2 text-[0.78rem] font-medium leading-snug text-ink">{lead.basis}</p>
                                        {lead.note && (
                                            <p className="mt-1 text-[0.7rem] leading-snug text-fg-muted">{lead.note}</p>
                                        )}
                                    </Stagger.Item>
                                ))}
                            </div>
                    ...
                    <div className="flex flex-col gap-6">
                        <div className="surface-blue p-6 md:p-8">
                            ...
                            {production.marketsNote && (
                                <p className="mb-5 text-[0.78rem] leading-[1.6] text-fg-muted">{production.marketsNote}</p>
                            )}
                            <MarketBars />
                        </div>
                        ...
                        <div className="relative min-h-[240px] flex-1 lg:min-h-0">
                            ...
                            <div className="absolute bottom-0 left-0 max-w-[85%] bg-blue px-5 py-4 md:px-6">
                                ...
                                <p className="mt-1.5 text-[0.68rem] leading-[1.5] text-white/80">
                                    {turnover.note}
                                </p>
                            </div>
                        </div>
